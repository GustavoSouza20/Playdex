...
(function ($) {
	$(document).ready(function () {
		...
		var $plugins = $('.bame-plugins'),
			$btn = $('.bame-admin-btn');

		// install action
		$plugins.on('click', '.bame-admin-btn[data-plugin-action="install"]', function (e) {

			e.preventDefault();

			if ($('.bame-admin-btn').hasClass('installing')) {
				return;
			}
			...
			$.ajax({
				...
				$btn.css('opacity', '1');
				if ($this.closest('.bame-plugin').length) {
					$this.closest('.row-actions').replaceWith('<div class="row-actions visible active"><span class="activate"><a href="#" class="button bame-admin-btn">Activated</a></span></div>');
				} else {
					$this.removeClass('installing').attr('data-plugin-action', 'deactivate').attr('href', response.substr(response.lastIndexOf('bame') + 6, response.length)).text('Deactivate').closest('.theme').addClass('active');
				}
			}).fail(function () {
		...
		// update action
		$plugins.on('click', '.bame-admin-btn[data-plugin-action="update"]', function (e) {
			e.preventDefault();
		...
		// activate action
		$plugins.on('click', '.bame-admin-btn[data-plugin-action="activate"]', function (e) {
		...
		// deactivate action
		$plugins.on('click', '.bame-admin-btn[data-plugin-action="deactivate"]', function (e) {
		...
		$('.whi-install-plugins').on('click', function (e) {
			...
			var $allPlugins = $installPluginsBtn.parent().next('.bame-plugins');
			var plugins = [];

			$allPlugins.find('.bame-plugin:not(:hidden)').each(function (index, element) {
				var $this = $(this);
				var $pluginActionBtn = $this.find('.bame-admin-btn');
