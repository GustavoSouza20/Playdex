...
class Admin {
			...
			if (!empty($response['message'])){
				...
				switch ($response['message']) {
					...
					case "subscription check failed":
						$code_message = !empty($response_code) ? 'HTTP response code: '.esc_html($response_code) : '';
    ...
    function maybeCreateOrUpdateContentTable() {
		...
		if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name
			...
			slug VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT '' NOT NULL,
			type VARCHAR(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT '' NOT NULL,
			aspect VARCHAR(50) DEFAULT '' NOT NULL,
			published TINYINT(1) DEFAULT 0 NOT NULL,
			content LONGTEXT NOT NULL,
			modified_at DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL,
			meta TEXT NOT NULL,
			func_ref TINYINT(1) DEFAULT 0,
			PRIMARY KEY (slug(191), type(50), published),
			KEY micro_content_lookup (published, slug(191), type(50), seq),
			KEY type_idx (type(50)),
			KEY published_idx (published),
			KEY aspect_idx (aspect(50)),
			KEY seq_idx (seq),
			KEY func_ref_idx (func_ref),
			KEY name_idx (name)
		) $charset_collate;";
			...
			update_option($this->html_mods_ver_name, $this->version);
			return true;
		}
	...
	function maybeCreateOrUpdateRevsTable() {
		...
		// only execute following code if table doesn't exist.
		...
		if( $wpdb->get_var( "SHOW TABLES LIKE '$table_name'" ) != $table_name or
			...
			update_option($this->micro_ver_name, $this->version);
        ...
        if ($pg_slug === 'html'){
	        ...
	        <div class="amender-notice">
	            ...
	            <div class="amender-upgrade">
	                ...
	                <span>
	                    <a target="_blank" href="https://themeover.com/pricing/?contentView=amender">Purchase a license</a> to publish
	                </span>
	            </div>
