...
class bame_recent_posts_widget extends WP_Widget {
        ...
        function __construct() {
            ...
            parent::__construct(
                ...
                'bame_recent_posts_widget',

                // Widget name will appear in UI

                esc_html__( 'Bame :: Recent Posts', 'bame' ),
                ...
                array(
                    ...
                    'description'                   => esc_html__( 'Add Recent Posts Widget', 'bame' ),
                )
            );
        }
                            ...
                            echo '<div class="recent-post">';
                                    ...
                                    echo '<div class="media-img">';
                                        echo '<a href="'.get_the_permalink().'">';
                                            echo bame_img_tag( array(
                                                "url"   => esc_url( get_the_post_thumbnail_url( null, 'bame_85X85' ) ),
                                            ) );
                                        echo '</a>';
                                    echo '</div>';
                                }
                                echo '<div class="media-body">';
                                    ...
                                    echo '<div class="recent-post-meta">'; 
                                        if($show_date){
                                            echo '<a href="'.esc_url( get_the_permalink() ).'"><i class="far fa-calendar"></i>'.esc_html( get_the_date() ).'</a>';
                                        }
                                    echo '</div>';
                                echo '</div>';
                            echo '</div>';
        ...
        public function form( $instance ) {
           ...
           //Post title Count
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'post_count' ); ?>"><?php _e( 'Number of Posts to show:' ,'bame'); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'post_title_count' ); ?>"><?php _e( 'Number of title word to show:' ,'bame'); ?></label>
        ...
        public function update( $new_instance, $old_instance ) {
            ...
            $instance['post_count']     = ( ! empty( $new_instance['post_count'] ) ) ? strip_tags( $new_instance['post_count'] ) : '2';
            $instance['post_title_count']     = ( ! empty( $new_instance['post_title_count'] ) ) ? strip_tags( $new_instance['post_title_count'] ) : '5';
            $instance['show_date']      = isset( $new_instance['show_date'] ) ? (bool) $new_instance['show_date'] : false;

            return $instance;
        }
    } // Class bame_recent_posts_widget ends here


    // Register and load the widget
    function bame_recent_posts_load_widget() {
        register_widget( 'bame_recent_posts_widget' );
    }
    add_action( 'widgets_init', 'bame_recent_posts_load_widget' );